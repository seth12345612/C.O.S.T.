    ...
    return (
      ...
      >
        <div className="flex items-start gap-3">
          ...
          >
            {isLocked ? t("Premium") : isDone ? t("Finalizat") : t("Marcheaz\u0103 ca finalizat")}
          </button>
        </div>
      </motion.div>
    );
      ...
      <div className="relative z-10 max-w-4xl mx-auto px-4 py-12">
        ...
        >
          <div className="inline-flex items-center gap-2 px-3 py-1 rounded-full bg-violet-500/10 border border-violet-500/20 text-violet-300 text-xs mb-4">
            <Zap size={12} /> {t("Provoc\u0103ri")}
          </div>
          <h1 className="text-3xl font-bold text-main mb-2">{t("Provoc\u0103ri")}</h1>
          <p className="text-dim">
            {t("{count} / {total} completate", { count: completed.length, total: allChallenges.length })}
          </p>
        </motion.div>
        ...
        {completedChallenges.length > 0 && (
          ...
          >
            <h2 className="text-lg font-bold text-green-400 mb-3 flex items-center gap-2">
              <CheckCircle size={16} /> {t("Finalizate")}
            </h2>
        ...
        >
          <h2 className="text-lg font-bold text-main mb-3 flex items-center gap-2">
            <Calendar size={16} className="text-cyan-400" /> {t("S\u0103pt\u0103m\u00e2nale")}
          </h2>
          <div className="space-y-2">
        ...
        >
          <h2 className="text-lg font-bold text-main mb-3 flex items-center gap-2">
            <Clock size={16} className="text-orange-400" /> {t("Lunare")}
          </h2>
          <div className="space-y-2">
        ...
        >
          <h2 className="text-lg font-bold text-main mb-3 flex items-center gap-2">
            <Crown size={16} className="text-purple-400" /> {t("Premium")}
            {!isAdvanced && <span className="text-[10px] text-muted font-normal ml-1">{t("(necesit\u0103 Premium Advanced)")}</span>}
          </h2>
          <div className="space-y-2">
