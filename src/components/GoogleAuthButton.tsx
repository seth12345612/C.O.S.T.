...
export function GoogleAuthButton({ compact }: { compact?: boolean }) {
  ...
  return user ? (
    <div className="flex items-center gap-3 bg-white/5 border border-white/10 rounded-full px-3 py-2 text-white text-xs">
      ...
      <div className="flex flex-col text-left">
        <span className="text-[11px] text-white/70">Bun venit</span>
        <span className="text-sm font-semibold truncate max-w-[120px]">{user.name}</span>
      </div>
      <button
        type="button"
        onClick={logout}
        className="rounded-full border border-white/10 bg-white/5 px-3 py-1 text-[10px] uppercase tracking-[0.14em] text-white/80 hover:bg-white/10 transition"
      >
    ...
    >
      ...
      <span className="tracking-wide">Conectare cu Google</span>
    </button>
  );
}
